...
describe('ContainerMixin', function() {
  var React = require('react/addons')
    ...
    , Container = React.createClass({
        ...
        render: function() { return(<div>{this.renderContainer()}</div>); }
      })
  ...
  describe('Mixin Interface', function() {
    it('should have a renderContainer method that renders the actual container', function() {
      expect(ContainerMixin.renderContainer).toBeDefined();
    });

    it('should expect items and itemTemplate as properties', function() {
      expect(ContainerMixin.propTypes.items).toBeDefined();
      expect(ContainerMixin.propTypes.itemTemplate).toBeDefined();
    });

    it('should set the items prop to a blank immutable list by default', function() {
      expect(ContainerMixin.getDefaultProps().items).toEqual([]);
    });

    it('should add state items for item, selected, and activeDropZone', function() {
      var container = TestUtils.renderIntoDocument(<Container />);
      expect(Immutable.List.isList(container.state.items)).toBeTruthy();
      expect(Immutable.Set.isSet(container.state.selected)).toBeTruthy();
      expect(container.state.activeDropZone).toBeDefined();
    });
  });
